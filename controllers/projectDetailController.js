      ...
      if (req.user && req.user.role === 'volunteer') {
        ...
        formattedProjects.forEach(project => {
          const application = applications.find(
            app => app.project_id.toString() === project._id.toString()
          );
          
          if (application) {
            project.application_status = application.status;
            project.has_applied = true;
            project.application_withdrawn = application.withdrawn_at !== null;
          } else {
            project.has_applied = false;
          }
        });
      }
...
const getProjectDetails = async (req, res) => {
  try {
    ...
    // Check if user is authenticated and add application status if they've applied
    if (req.user && req.user.role === 'volunteer') {
      ...
      if (application) {
        formattedProject.application_status = application.status;
        formattedProject.has_applied = true;
        formattedProject.application_withdrawn = application.withdrawn_at !== null;
        formattedProject.application_id = application._id;
        formattedProject.application_date = application.date_applied;
        formattedProject.organizer_feedback = application.organizer_feedback;
      } else {
        formattedProject.has_applied = false;
      }
    }
...
// @desc    Get project application status for the current volunteer
// @route   GET /api/project-details/:id/application-status
// @access  Private (Volunteers only)
const getApplicationStatus = async (req, res) => {
  try {
    // Check if user is a volunteer
    if (req.user.role !== 'volunteer') {
      return sendResponse(res, 403, 'Only volunteers can check application status');
    }
    ...
    if (!application) {
      return sendResponse(res, 404, 'You have not applied for this project');
    }
    
    return sendResponse(res, 200, 'Application status retrieved successfully', {
      status: application.status,
      date_applied: application.date_applied,
      withdrawn: application.withdrawn_at !== null,
      feedback: application.organizer_feedback
    });
  } catch (error) {
    console.error(error);
    return sendResponse(res, 500, 'Server error');
...
const getOrganizerProjects = async (req, res) => {
  try {
    ...
    // Add application statistics to each project
